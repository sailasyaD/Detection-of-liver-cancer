...
% Last Modified by GUIDE v2.5 25-Apr-2019 19:35:45
...
handles.TexFeature=feat;
save TextureFeat feat
% Update handles structure
   guidata(hObject, handles);
...
load Up2Seg u1
jkk=1;
regionFeat=regionprops(u1,'All');
for jk=1:length(regionFeat)
    if regionFeat(jk).Area>10
    Area(jkk)=regionFeat(jk).Area;
    Centroid(jkk)=mean(regionFeat(jk).Centroid);
    ConvexArea(jkk)=regionFeat(jk).ConvexArea;
    Eccentricity(jkk)=regionFeat(jk).Eccentricity;
    EquivDiameter(jkk)=regionFeat(jk).EquivDiameter;
    EulerNumber(jkk)=regionFeat(jk).EulerNumber;
    Extent(jkk)=regionFeat(jk).Extent;
    Extrema(jkk)=mean2(regionFeat(jk).Extrema);
    MajorAxisLength(jkk)=regionFeat(jk).MajorAxisLength;
    MinorAxisLength(jkk)=regionFeat(jk).MinorAxisLength;
    Orientation(jkk)=regionFeat(jk).Orientation;
    Perimeter(jkk)=regionFeat(jk).Perimeter;
    Solidity(jkk)=regionFeat(jk).Solidity;
    jkk=jkk+1;
    end
end
Area=mean(Area);
Centroid=mean(Centroid);
ConvexArea=mean(ConvexArea);
Eccentricity=mean(Eccentricity);
EquivDiameter=mean(EquivDiameter);
EulerNumber=mean(EulerNumber);
Extent=mean(Extent);
Extrema=mean(Extrema);
MajorAxisLength=mean(MajorAxisLength);
MinorAxisLength=mean(MinorAxisLength);
Orientation=mean(Orientation);
Perimeter=mean(Perimeter);
Solidity=mean(Solidity);
ShapeDensity=[Area Centroid ConvexArea Eccentricity EquivDiameter EulerNumber Extent Extrema MajorAxisLength MinorAxisLength ...
    Orientation Perimeter Solidity];
    

set(handles.edit14,'string',Area);
set(handles.edit15,'string',Centroid);
set(handles.edit16,'string',ConvexArea);
set(handles.edit17,'string',Eccentricity);
set(handles.edit18,'string',EquivDiameter);
set(handles.edit19,'string',EulerNumber);
set(handles.edit20,'string',Extent);
set(handles.edit21,'string',Extrema);
set(handles.edit22,'string',MajorAxisLength);
set(handles.edit23,'string',MinorAxisLength);
set(handles.edit24,'string',Orientation);
set(handles.edit25,'string',Perimeter);
set(handles.edit26,'string',Solidity);
handles.GeoFeature=ShapeDensity;
save ShapeDenFeat ShapeDensity
% Update handles structure
   guidata(hObject, handles);
...
ImGraySeg=segg;
%%%% input Verification   %%%%%%%%%%%%
varhi=255; varlo=0; % variable limits
ff='SelectPixel'; % objective function
npar=128; % number of optimization variables
% II Stopping criteria
maxit=50; % max number of iterations
mincost=0.1; % minimum cost
%_______________________________________________________
% III GA parameters
popsize=128; % set population size
mutrate=.2; % set mutation rate
selection=0.9; % crossove probability
...
cost=feval(ff,par,ImGraySeg); % calculates population cost
% using ff
...
axes(handles.axes1)
  bar(BesC,'r');
  xlabel('No of Iterations in GA','Color','w');
%   ylabel('Selected Feature Value');
  title('GA selected Feature','Color','w');
  save GaFeat BesC
